pcall(function() 
  ...
  local function updateSizes()
    ...
    mainFrame.Size = UDim2.new(0, viewport.X * 0.6, 0, viewport.Y * 0.4)
    mainFrame.Position = UDim2.new(0.5, -mainFrame.Size.X.Offset / 2, 0.5, -mainFrame.Size.Y.Offset / 2)
    local iconSizeVal = viewport.X * 0.05
    iconButton.Size = UDim2.new(0, iconSizeVal, 0, iconSizeVal)
    iconButton.Position = UDim2.new(0.95, -iconSizeVal, 0.95, -iconSizeVal)
  end
  -- Create ScreenGui 
  local screenGui = Instance.new("ScreenGui") 
  ...
  searchBar.Size = UDim2.new(0.45, 0, 0.1, 0) -- Adjusted
  ...
  categoryFrame.Name = "CategoryScroll"
  categoryFrame.Size = UDim2.new(0.45, 0, 0.1, 0)
  categoryFrame.Position = UDim2.new(0.05, 0, 0.2, 0)
  ...
  availableLabel.Text = "Available Animations"
  availableLabel.Font = Enum.Font.SourceSansBold
  availableLabel.TextScaled = true
  availableLabel.TextColor3 = Color3.fromRGB(255, 255, 255)
  availableLabel.BackgroundTransparency = 1
  availableLabel.Size = UDim2.new(0.45, 0, 0.1, 0)
  availableLabel.Position = UDim2.new(0.05, 0, 0.3, 0)
  availableLabel.Parent = mainFrame

  -- Create Available Scroll Frame
  local scrollFrame = Instance.new("ScrollingFrame")
  scrollFrame.Name = "AvailableScroll"
  scrollFrame.Size = UDim2.new(0.45, 0, 0.5, 0) -- 50% of frame's height
  scrollFrame.Position = UDim2.new(0.05, 0, 0.4, 0)
  scrollFrame.BackgroundColor3 = Color3.fromRGB(40, 40, 40)
  scrollFrame.BorderSizePixel = 0
  scrollFrame.ScrollBarThickness = 8 -- Visible scroll
  scrollFrame.ScrollingDirection = Enum.ScrollingDirection.Y -- Vertical scrolling only
  scrollFrame.CanvasSize = UDim2.new(0, 0, 0, 0)
  scrollFrame.Parent = mainFrame
  local availableListLayout = Instance.new("UIListLayout")
  availableListLayout.Parent = scrollFrame
  availableListLayout.SortOrder = Enum.SortOrder.LayoutOrder
  availableListLayout.Padding = UDim.new(0, 5)

  -- Used label
  local usedLabel = Instance.new("TextLabel")
  usedLabel.Text = "Used Animations"
  usedLabel.Font = Enum.Font.SourceSansBold
  usedLabel.TextScaled = true
  usedLabel.TextColor3 = Color3.fromRGB(255, 255, 255)
  usedLabel.BackgroundTransparency = 1
  usedLabel.Size = UDim2.new(0.45, 0, 0.1, 0)
  usedLabel.Position = UDim2.new(0.5, 0, 0.2, 0)
  usedLabel.Parent = mainFrame

  -- Create Used Scroll Frame
  local usedScroll = Instance.new("ScrollingFrame")
  usedScroll.Name = "UsedScroll"
  usedScroll.Size = UDim2.new(0.45, 0, 0.6, 0) -- 60% of frame's height
  usedScroll.Position = UDim2.new(0.5, 0, 0.3, 0)
  usedScroll.BackgroundColor3 = Color3.fromRGB(40, 40, 40)
  usedScroll.BorderSizePixel = 0
  usedScroll.ScrollBarThickness = 8 -- Visible scroll
  usedScroll.ScrollingDirection = Enum.ScrollingDirection.Y -- Vertical scrolling only
  usedScroll.CanvasSize = UDim2.new(0, 0, 0, 0)
  usedScroll.Parent = mainFrame
  local usedListLayout = Instance.new("UIListLayout")
  usedListLayout.Parent = usedScroll
  usedListLayout.SortOrder = Enum.SortOrder.LayoutOrder
  usedListLayout.Padding = UDim.new(0, 5)
  ...
  -- Icon click: show main frame, hide icon
  iconButton.MouseButton1Click:Connect(function()
    mainFrame.Visible = true
    mainFrame.Size = getScaledSize(0, 0)
    TweenService:Create(mainFrame, TweenInfo.new(0.3, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {Size = getScaledSize(0.6, 0.4)}):Play()
    iconButton.Visible = false
  end)

  -- Close button click: hide main frame, show icon
  closeButton.MouseButton1Click:Connect(function()
    TweenService:Create(mainFrame, TweenInfo.new(0.3, Enum.EasingStyle.Quad, Enum.EasingDirection.In), {Size = getScaledSize(0, 0)}):Play()
    task.wait(0.3)
    mainFrame.Visible = false
    iconButton.Visible = true
  end)

  camera:GetPropertyChangedSignal("ViewportSize"):Connect(updateSizes)
  updateSizes()
  ...
  local function createTheButton(text, callback) 
    ...
    task.wait()
    scrollFrame.CanvasSize = UDim2.new(0, 0, 0, availableListLayout.AbsoluteContentSize.Y) -- Update canvas size 
  end 
  -- Example button 
  ...
  searchBar:GetPropertyChangedSignal("Text"):Connect(function() 
    ...
    task.wait()
    scrollFrame.CanvasSize = UDim2.new(0, 0, 0, availableListLayout.AbsoluteContentSize.Y) 
  end) 
  ...
  local function Wait2() 
    ...
    local character = player.Character or player.CharacterAdded:wait(0.1) 
    local humanoid = character:WaitForChild("Humanoid") 
    while humanoid:GetState() == Enum.HumanoidStateType.Dead or humanoid:GetState() == Enum.HumanoidStateType.Ragdoll do 
  ...
  task.wait(0.1) -- Define the animations table with animation IDs 
  local Animations = { 
    Idle = { 
  ...
  local function getAnimName(animationType, animationId)
    local anims = Animations[animationType]
    for name, id in pairs(anims) do
      if type(id) == "table" then
        if #id == #animationId and id[1] == animationId[1] and id[2] == animationId[2] then return name end
      else
        if id == animationId then return name end
      end
    end
    return "Unknown"
  end
  local function resetAnimation(animationType)
    local defaultId = (animationType == "Idle") and {"0","0"} or "0"
    setAnimation(animationType, defaultId)
    if animationType == "Idle" then
      CurrentIdle = "None"
    elseif animationType == "Walk" then
      CurrentWalk = "None"
  ...
  local function updateUsedList()
    for _, child in ipairs(usedScroll:GetChildren()) do
      if not child:IsA("UIListLayout") then child:Destroy() end
    end
    local types = {"Idle", "Walk", "Run", "Jump", "Fall", "SwimIdle", "Swim", "Climb"}
    for _, t in ipairs(types) do
      local curr
      if t == "Idle" then curr = CurrentIdle
      elseif t == "Walk" then curr = CurrentWalk
      elseif t == "Run" then curr = CurrentRun
      elseif t == "Jump" then curr = CurrentJump
      elseif t == "Fall" then curr = CurrentFall
      elseif t == "SwimIdle" then curr = CurrentSwimIdle
      elseif t == "Swim" then curr = CurrentSwim
      elseif t == "Climb" then curr = CurrentClimb
      end
      if curr ~= "None" then
        ...
        cancelBtn.Size = UDim2.new(0.2, 0, 1, 0)
        cancelBtn.Position = UDim2.new(0.8, 0, 0, 0)
        cancelBtn.BackgroundColor3 = Color3.fromRGB(200, 50, 50)
        cancelBtn.TextColor3 = Color3.fromRGB(255, 255, 255)
        cancelBtn.TextScaled = true
        cancelBtn.Font = Enum.Font.SourceSansBold
        cancelBtn.Parent = itemFrame
    ...
    task.wait()
    usedScroll.CanvasSize = UDim2.new(0, 0, 0, usedListLayout.AbsoluteContentSize.Y)
  end
  local HttpService = game:GetService("HttpService") 
  ...
  local function createButton(name, animationType, animationId) 
    createTheButton(name .. " - " .. animationType , function() 
      setAnimation(animationType, animationId) 
      UpdaLabel(animationType, name)
      Notify(animationType,name,1) 
      updateUsedList()
    end) 
  end 
  ...
  local function createCategoryButton(text)
    ...
    button.MouseButton1Click:Connect(function()
      ...
      populateAvailable(text)
    end)
    categoryFrame.CanvasSize = UDim2.new(0, categoryLayout.AbsoluteContentSize.X, 0, 0)
  end
  local categories = {"Idle", "Walk", "Run", "Jump", "Fall", "SwimIdle", "Swim", "Climb", "Emotes", "Donates"}
  for _, cat in ipairs(categories) do
    createCategoryButton(cat)
  end
  local function populateAvailable(category)
    for _, child in ipairs(scrollFrame:GetChildren()) do
      if child:IsA("TextButton") then child:Destroy() end
    end
    buttons = {}
    if Animations[category] then
      for name, ids in pairs(Animations[category]) do
        task.wait((1/3) * game:GetService("RunService").RenderStepped:Wait())
        createButton(name, category, ids)
      end
  ...
  populateAvailable(currentCategory)
