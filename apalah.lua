pcall(function()
    ...
    SanLabel.Text = "San ANIMATOR"
    SanLabel.Font = Enum.Font.GothamBlack
    SanLabel.TextScaled = true
    SanLabel.TextColor3 = Color3.fromRGB(255, 255, 255)
    SanLabel.BackgroundTransparency = 1
    SanLabel.Size = UDim2.new(0.7, 0, 0.1, 0)
    SanLabel.Position = UDim2.new(0, 5, 0, 5)
    SanLabel.Parent = animListFrame
    ...
    hideButton.Position = UDim2.new(1, -75, 0, 5)
    ...
    local function local function setAnimation(animationType, animationId, animName)
    local speaker = Players.LocalPlayer
    local Char = speaker.Character
    if not Char then 
    ...
    -- Tunggu hingga Animate script tersedia
    local Animate = Char:FindFirstChild("Animate")
    if not Animate then
        Notify("Waiting", "Waiting for Animate script...", 1)
        repeat 
            task.wait(0.5)
    ...
    local function applySavedAnimations(character)
        -- Tunggu hingga karakter siap
        local humanoid = character:WaitForChild("Humanoid")
        
        -- Tunggu hingga karakter hidup dan tidak dalam state dead/ragdoll
        ...
        -- Tunggu sedikit lebih lama untuk memastikan Animate script sudah dimuat
        task.wait(2)
        
local hasSavedAnimations = false
    for animType, animId in pairs(lastAnimations) do
        if animType == "Idle" or animType == "Walk" or animType == "Run" or 
           animType == "Jump" or animType == "Fall" or animType == "Swim" or 
           animType == "SwimIdle" or animType == "Climb" then
    ...
    if not hasSavedAnimations then
        ...
        -- Terapkan animasi yang disimpan
        if lastAnimations.Idle and lastAnimations.IdleName then
        task.wait(0.2)
        setAnimation("Idle", lastAnimations.Idle, lastAnimations.IdleName)
    end
    
    if lastAnimations.Walk and lastAnimations.WalkName then
        task.wait(0.2)
        setAnimation("Walk", lastAnimations.Walk, lastAnimations.WalkName)
    end
    
    if lastAnimations.Run and lastAnimations.RunName then
        task.wait(0.2)
        setAnimation("Run", lastAnimations.Run, lastAnimations.RunName)
    end
    
    if lastAnimations.Jump and lastAnimations.JumpName then
        task.wait(0.2)
        setAnimation("Jump", lastAnimations.Jump, lastAnimations.JumpName)
    end
    
    if lastAnimations.Fall and lastAnimations.FallName then
        task.wait(0.2)
        setAnimation("Fall", lastAnimations.Fall, lastAnimations.FallName)
    end
    
    if lastAnimations.Swim and lastAnimations.SwimName then
        task.wait(0.2)
        setAnimation("Swim", lastAnimations.Swim, lastAnimations.SwimName)
    end
    
    if lastAnimations.SwimIdle and lastAnimations.SwimIdleName then
        task.wait(0.2)
        setAnimation("SwimIdle", lastAnimations.SwimIdle, lastAnimations.SwimIdleName)
    end
    
    if lastAnimations.Climb and lastAnimations.ClimbName then
        task.wait(0.2)
        setAnimation("Climb", lastAnimations.Climb, lastAnimations.ClimbName)
    end
    
    Notify("Auto-Apply", "All saved animations applied!", 2)
end
    ...
    -- Daftarkan event untuk auto-apply animasi saat respawn
    Players.LocalPlayer.CharacterAdded:Connect(function(character)
    Notify("Character", "New character detected, applying animations...", 2)
    -- Tunggu lebih lama sebelum apply animasi
    task.wait(3)
...
if Players.LocalPlayer.Character then
    task.wait(2) -- Tunggu sedikit di awal
