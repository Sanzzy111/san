pcall(function()
    ...
    local function applySavedAnimations(character)
        ...
        for animType, animId in pairs(lastAnimations) do
            if animType == "Idle" and lastAnimations.IdleName then
                setAnimation("Idle", animId, lastAnimations.IdleName)
            elseif animType == "Walk" and lastAnimations.WalkName then
                setAnimation("Walk", animId, lastAnimations.WalkName)
            elseif animType == "Run" and lastAnimations.RunName then
                setAnimation("Run", animId, lastAnimations.RunName)
            elseif animType == "Jump" and lastAnimations.JumpName then
                setAnimation("Jump", animId, lastAnimations.JumpName)
            elseif animType == "Fall" and lastAnimations.FallName then
                setAnimation("Fall", animId, lastAnimations.FallName)
            elseif animType == "Swim" and lastAnimations.SwimName then
                setAnimation("Swim", animId, lastAnimations.SwimName)
            elseif animType == "SwimIdle" and lastAnimations.SwimIdleName then
                setAnimation("SwimIdle", animId, lastAnimations.SwimIdleName)
            elseif animType == "Climb" and lastAnimations.ClimbName then
                setAnimation("Climb", animId, lastAnimations.ClimbName)
            end
        end
        
        Notify("Auto-Apply", "Animations applied after respawn", 2)
    end
    ...
    Players.LocalPlayer.CharacterAdded:Connect(function(character)
        -- Tunggu sedikit sebelum apply animasi
        task.wait(1)
