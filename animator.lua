pcall(function()
    ...
    local function createTheButton(text, callback)
        ...
        button.TextColor3 = Color3.fromRGB(255,  ascended
...
System: 255, 255)
    ...
    local function ResetSwim()
        ...
        pcall(function()
            local Animate = Char:FindFirstChild("Animate")
            if Animate and Animate.swim then
    ...
    local function ResetSwimIdle()
        ...
        pcall(function()
            local Animate = Char:FindFirstChild("Animate")
            if Animate and Animate.swimidle then
    ...
    local function setAnimation(animationType, animationId, animName)
        ...
        local Char = speaker.Character
        if not Char then
            Notify("Error", "No character found for " .. animationType, 3)
            return
        end
        local Animate = Char:FindFirstChild("Animate")
        if not Animate then
            Notify("Error", "Animate script not found for " .. animationType, 3)
            return
        end
        freeze()
        StopAnim()
    ...
    local function applySavedAnimations(character)
        ...
        if not humanoid or not animateScript then
            Notify("Error", "Failed to find Humanoid or Animate script", 3)
            return
        end
        ...
        task.wait(0.5) -- Additional delay to ensure character is fully loaded
        Notify("Debug", "Applying saved animations to character", 3)
        for animType, animId in pairs(lastAnimations) do
            if animType ~= "IdleName" and animType ~= "WalkName" and animType ~= "RunName" and animType ~= "JumpName" and
               animType ~= "FallName" and animType ~= "SwimIdleName" and animType ~= "SwimName" and animType ~= "ClimbName" then
                Notify("Debug", "Applying " .. animType .. ": " .. (lastAnimations[animType .. "Name"] or "None"), 3)
                setAnimation(animType, animId, lastAnimations[animType .. "Name"])
            end
        end
        Notify("Debug", "Finished applying saved animations", 3)
    end
    ...
    Notify("Changelog", "Fixed auto-apply animations on respawn, UI spacing, and status text overflow", 30)
end)
